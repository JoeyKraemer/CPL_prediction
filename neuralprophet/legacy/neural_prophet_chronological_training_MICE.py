...
# Setup logging
def setup_logging(model_type="NP_MICE"):
    """Setup logging to timestamped file"""
    logs_dir = Path("prophet/logs")
    logs_dir.mkdir(parents=True, exist_ok=True)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = logs_dir / f"{timestamp}_{model_type}.log"
...
class SimpleNeuralProphetForecaster:
    ...
    def load_and_prepare_data(
        ...
        """Load and prepare CPL data for NeuralProphet with proper train/test split, feature engineering, and optional MICE imputation"""
        # Load dataset
        ...
        # Apply MICE imputation if enabled
        if use_mice:
            print("Applying MICE imputation...")
            train_data = self._apply_mice_imputation(train_data)

        # Define OPTIMIZED feature columns (reduced from 23 to 11 external regressors)
    ...
    def _fill_date_gaps(self, df):
        """Fill missing dates in time series with NaN for CPL values"""
        if len(df) == 0:
            return df
        ...
        # Reindex to include all dates (missing dates will have NaN)
        df = df.reindex(full_range)
        df.index.name = "date"

        return df.reset_index()

    def _apply_mice_imputation(self, train_data):
        """Apply MICE imputation to fill gaps in training data only."""
        # Store original length
        original_train_len = len(train_data)

        # Fill date gaps in training data
        train_filled = self._fill_date_gaps(train_data)

        gaps_filled = len(train_filled) - original_train_len

        if gaps_filled > 0:
            print(f"  Filled {gaps_filled} date gaps in training data")

        # Apply MICE only if there are missing values
        if train_filled["Cost_per_Lead_anon"].isna().sum() > 0:
            missing_count = train_filled["Cost_per_Lead_anon"].isna().sum()

            # Create sklearn IterativeImputer (MICE)
            imputer = IterativeImputer(max_iter=10, random_state=42, verbose=0)

            # Fit and transform training data
            train_filled["Cost_per_Lead_anon"] = imputer.fit_transform(
                train_filled[["Cost_per_Lead_anon"]]
            ).flatten()
    ...
    def _engineer_features(self, df):
        """Engineer OPTIMIZED feature set - reduced from 23 to 11 features to avoid redundancy with n_lags"""
        df = df.copy()
        ...
        # Campaign budget/limit features
        if "campaign_max_day" in df.columns:
            df["max_day_budget"] = df["campaign_max_day"].fillna(0)
        else:
            df["max_day_budget"] = 0

        if "campaign_max_total" in df.columns:
            df["max_total_budget"] = df["campaign_max_total"].fillna(0)
        else:
            df["max_total_budget"] = 0

        # 3. VOLATILITY INDICATORS (capture uncertainty, not redundant with lags)
        df["rolling_std_3"] = (
            df["Cost_per_Lead_anon"].rolling(window=3, min_periods=1).std().fillna(0)
        )
        df["rolling_std_7"] = (
            df["Cost_per_Lead_anon"].rolling(window=7, min_periods=1).std().fillna(0)
        )

        # 4. INTERACTION FEATURES (non-linear relationships)
        df["CTR_CR_interaction"] = df["CTR"] * df["Conversion_Rate"]

        # NOTE: Removed redundant features:
        # - Manual lag features (CPL_lag_1/3/7, CTR_lag_1, etc.) -> n_lags=7 already creates these
        # - Rolling means (rolling_mean_3/7) -> Linear combination of lags
        # - Some temporal features (month, quarter) -> Captured by yearly_seasonality
        # - Relative features (CPL_vs_avg, days_since_start) -> Lower predictive value
    ...
    def train_model(
        ...
        # Create NeuralProphet model with external regressors
        self.model = NeuralProphet(
            ...
            loss_func="Huber",  # Huber loss = robust to outliers
        )
...
def evaluate_all_campaigns(
    ...
    for idx, campaign in enumerate(valid_campaigns, 1):
        try:
            ...
            # Load campaign data with MICE imputation
            split_info = forecaster.load_and_prepare_data(
                csv_path=csv_path,
                campaign_name=campaign,
                use_train_test_split=True,
                use_mice=True,  # Enable MICE imputation to handle gaps
            )

            # Note: Gap detection removed - MICE handles missing dates automatically
...
def main(mode="fast"):
    ...
    """
    # Setup logging
    log_file = setup_logging(model_type="NP_MICE")
